...
Estado a 5 de septiembre de 2026: la documentación de arquitectura está lista;
...
| 2 | Esqueleto del repo: `backend/` con `cmd/`, `internal/platform/`, `Dockerfile` multi-stage, `Makefile` | — | Pendiente |
| 3 | `docker-compose.yml` con los servicios de `disenos/contexto-y-componentes.md` y healthchecks | — | Pendiente |
| 4 | Migraciones `0001`–`00NN` según `disenos/modelo-de-datos.md`, con los triggers de inmutabilidad y auditoría | — | Pendiente |
| 5 | Plataforma: errores RFC 9457, middleware de idempotencia, rate limiting, logs, métricas, OTel | — | Pendiente |
| 6 | `identity` + `admin` + `audit` | — | Pendiente |
