...
| **`badges`** | 4 / 5 | Un worker emite la insignia al aprobar, **única por inscripción** con dos barreras: clave determinista del trabajo y restricción `UNIQUE`. Publica su **imagen SVG** en el bucket de lectura anónima y la expone como `image_url`. Verificación **pública sin el correo** del estudiante, tampoco en la imagen. Revocación auditada. | `GET /badges/{id}`. |
| **`media`** | 11 / 11 | Carga multipart con URLs prefirmadas: **los bytes no pasan por la API**. **Reanudación real**: consulta qué partes faltan y sube solo esas, con 24 h de ventana. Verificación en servidor: recalcula el SHA-256 leyendo del bucket y detecta el MIME por *magic bytes*. Lo rechazado va a cuarentena con el original intacto. **Escaneo antimalware con ClamAV** antes de procesar nada. **Transcodificación a HLS** en un worker aparte, con escalera sin *upscaling*, póster y original intacto. Manifiesto maestro y playlists con segmentos firmados, sin abrir el bucket. **Sesión de reproducción** por inscripción, que comprueba el derecho y emite una credencial de 30 min. | Nada del alcance mínimo. En GCP, la credencial pasa a cookie de CDN (ADR-0015, D2). Ver [`media-plan.md`](media-plan.md). |
...
| `media.scan` | ✅ | ClamAV por INSTREAM; infectado → cuarentena y auditoría |
| `media.transcode_hls` | ✅ | FFmpeg → HLS, escalera sin *upscaling* |
| `media.poster` | ✅ | Fotograma del segundo 3, dentro de `media.transcode_hls` |
